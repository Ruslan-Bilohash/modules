        ...
        <div class="row g-4">
            ...
            <div class="col-md-6 mt-4">
                <div class="form-check">
                    <input type="checkbox" name="published" id="published" class="form-check-input" checked>
...
<script>
    ...
    function updateSEO(element) {
        ...
        switch(id.replace(/_.+$/, '')) {
            case 'title': case 'meta_title': case 'og_title': case 'twitter_title':
                ...
                break;
            case 'short_desc': case 'meta_desc': case 'og_desc': case 'twitter_desc':
    ...
    function updateUrlPreview(element) {
        ...
        if (title) {
            const url = '<?php echo htmlspecialchars($_SERVER['HTTP_HOST']); ?>/news/' + transliterate(title);
    ...
    function initializeProgressBars() {
        document.querySelectorAll('input[id], textarea[id]:not(.tinymce)').forEach(input => {
            input.addEventListener('input', () => updateSEO(input));
            input.addEventListener('change', () => updateSEO(input));
            updateSEO(input);
        });

        // Отдельная обработка для TinyMCE
    ...
    // Инициализация предпросмотра URL
    const titleInput = document.getElementById('title');
    if (titleInput) {
        titleInput.addEventListener('input', () => updateUrlPreview(titleInput));
        updateUrlPreview(titleInput);
    }
</script>
</body>
</html>